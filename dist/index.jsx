...
export default ({ Button }) => {
    ...
    const cellRenderer = (header, row) => {
        ...
        if (header === "repo_id") {
            return (<a href={`https://huggingface.co/${cell}`} target="_blank" rel="noreferrer" className="hover:underline">
          {cell}
        </a>);
        }
        if (header === "delete") {
            return (<Button variant="destructive" onClick={() => deleteRevision(row["commit_hash"])}>
          Delete
      ...
      <div className="overflow-x-scroll w-full">
        <div className="align-middle inline-block w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
          <table className="w-full">
            <thead>
              <tr>
                {finalHeaderSelection.map((header) => (<th key={header} className="px-2 py-3 border-b border-gray-200 text-xs font-medium text-gray-500 cursor-pointer" onClick={() => {
                setSortKey(header);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {rows.map((row) => (<tr key={row["commit_hash"]}>
                  {finalHeaderSelection.map((header, index) => (<td key={index} className="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
